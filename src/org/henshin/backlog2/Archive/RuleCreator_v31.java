...
public class RuleCreator_v31 {
	public static CModule module = new CModule("backlog_v31");
	private static final Logger LOGGER = Logger.getLogger(RuleCreator_v31.class.getName());
	...
	public static void main(String[] args) {
		...
		RuleCreator_v31 ruleCreator = new RuleCreator_v31();
			...
			try {
				...
				if (jsonObject.has("Persona")) {
					...
					for (int i = 0; i < persona.length(); i++) {
						...
						nodePersona.createAttribute("name", "\"" + persona.getString(i).toLowerCase(Locale.ROOT) + "\"");
					}
				} else {
				...
				if (jsonObject.has("Action")) {
					...
					if (action.has("Primary Action")) {
						...
						for (int i = 0; i < primaryAction.length(); i++) {
							...
							// CNode cNode = userStory.createNode("Primary Action");
							cNode.createAttribute("name", "\"" + primaryAction.getString(i).toLowerCase(Locale.ROOT) + "\"",
									"delete");
					...
					if (action.has("Secondary Action")) {
						...
						for (int i = 0; i < secondaryAction.length(); i++) {
							...
							// CNode cNode = userStory.createNode("Secondary Action");
							cNode.createAttribute("name", "\"" + secondaryAction.getString(i).toLowerCase(Locale.ROOT) + "\"",
									"delete");
					...
					if (entity.has("Primary Entity")) {
						...
						for (int i = 0; i < primaryEntity.length(); i++) {
							...
							if (checkEntityIsTarget(primaryEntity.getString(i), targetsArray)) {
								...
								cNode.createAttribute("name", "\"" + primaryEntity.getString(i).toLowerCase(Locale.ROOT) + "\"",
										"delete");
					...
					if (entity.has("Secondary Entity")) {
						...
						for (int i = 0; i < secondaryEntity.length(); i++) {
							...
							if (checkEntityIsTarget(secondaryEntity.getString(i), targetsArray)) {
								...
								cNode.createAttribute("name", "\"" + secondaryEntity.getString(i).toLowerCase(Locale.ROOT) + "\"",
										"delete");
